...
for (i in cl_names) {
  ...
  # Initialising
  DGE_list_LFC0 <- vector(mode = "list", length = 2)
...
barcode_2 <- gsInfo(gsea_output_MCF7M$PAR_LY.ARC_vs_PAR_LY, geneSetID = c("HALLMARK_E2F_TARGETS"))
...
barcode_2$group <- c("PAR_LY.ARC_vs_PAR_LY")
...
incyclix_col <- c(
  ...
  "PAR_LY.ARC_vs_PAR_LY" = "#872464"
)
# "LYFR_DMSO_vs_PAR_LY" = "#FFC541")


# incyclix_col %>% show_col
comparison_group <- c("PAR_LY_vs_PAR_DMSO", "PAR_LY.ARC_vs_PAR_LY")
...
barcode_2 <- gsInfo(gsea_output_T47D$PAR_LY.ARC_vs_PAR_LY, geneSetID = c("HALLMARK_E2F_TARGETS"))
...
a2 <- barcode_1_2 %>% ggplot(aes(x = x, y = runningScore, col = group)) +
  geom_line(alpha = 0.95, size = 3) +
  # geom_point(aes(color = Treatment), alpha = 0.5, size = 0.7) +
  # scale_color_manual(values = c(LYvsDMSO = "#00B938", LYSRvsLY = "#609CFF"),
  # labels = c(LYvsDMSO = "Abema vs DMSO", LYSRvsLY = "Abema+SR11302 vs Abema")) +
...
pdf("preventresist_MCF7M_E2Ftarget_barcode_LYR_DMSO_vs_PAR_LY.pdf",
...
pdf("preventresist_T47D_E2Ftarget_barcode_LYR_DMSO_vs_PAR_LY.pdf",
  width = 8, # 5 x 300 pixels
  height = 5
) # 300 pixels per inch)        # smaller font size
